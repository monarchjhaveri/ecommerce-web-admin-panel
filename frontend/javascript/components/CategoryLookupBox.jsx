    ...
    render: function() {
        if (this.state.selectedResult) {
            ...
            if (this.state.selectedResult.parent) {
                ...
                rows.push(<tr className="clickable" key={this.state.selectedResult.parent.name} onClick={_selectBoxFunctionGenerator(this, parent)}><td>Parent:<br/>{parent.name}</td></tr>);
            }

            var childRows = [];
            if (this.state.selectedResult.children instanceof Array) {
                var children = this.state.selectedResult.children;
                childRows.push(<tr key={"children-header"}><td><h3>Children</h3></td></tr>);
                for (var i = 0; i < children.length; i++) {
                    childRows.push(<tr className="clickable" key={children[i].id} onClick={_selectBoxFunctionGenerator(this, children[i])}><td>{children[i].name}</td></tr>);
                }
            }
            return (
